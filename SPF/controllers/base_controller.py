...
class SPFBaseController(app_manager.OSKenApp):
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # Multipath & ECMP helpers (Topik 2)
    # ──────────────────────────────────────────────────────────────────────────
    ...
    def _alloc_group_id(self, src_mac, dst_mac):
        ...
        candidate = int(hashlib.md5(seed).hexdigest()[:8], 16) & 0x7FFFFFFF
        if candidate == 0:
            candidate = 1
        return candidate

    def _install_select_group(self, datapath, group_id, out_ports, weights=None):
        """Install or replace OpenFlow SELECT group with configurable weights (Topik 2)."""
        parser = datapath.ofproto_parser
        ofproto = datapath.ofproto
        ...
        # Modifikasi Logika Pembobotan (Weight / Hash Distribution) - Topik 2
        # Secara default akan memberikan weight merata jika tidak ditentukan, 
        # namun dapat dikonfigurasi secara dinamis.
        if weights is None:
        ...
        buckets = [
            ...
            for p, w in zip(unique_ports, weights)
        ]
        ...
        self.logger.debug("[ECMP-GROUP] s%d group=%d ports=%s weights=%s",
                          datapath.id, group_id, unique_ports, weights)
        return True

    def _install_group_flow(self, datapath, in_port, src_mac, dst_mac, group_id):
        """Install ingress flow that forwards to a SELECT group."""
        parser = datapath.ofproto_parser
        ofproto = datapath.ofproto
        match = parser.OFPMatch(in_port=in_port, eth_src=src_mac, eth_dst=dst_mac)
